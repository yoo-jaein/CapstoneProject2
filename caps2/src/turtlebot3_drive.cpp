...
Turtlebot3Drive::~Turtlebot3Drive()
{
  updatecommandVelocity(0.0, 0.0, 0);
  ros::shutdown();
}
...
bool Turtlebot3Drive::init()
{
  ...
  cam_sub = nh_.subscribe("alert", 10, &Turtlebot3Drive::camCallback, this);
...
void Turtlebot3Drive::updatecommandVelocity(double linear, double angular, int num)
{
...
void Turtlebot3Drive::camCallback(const std_msgs::String::ConstPtr &msg)
{
	geometry_msgs::Twist cmd_vel;
	if(msg->data.compare("0") == 0){
		ros::Time::init();	
		std::cout << "[INFO] No mask detected" << std::endl;
		//updatecommandVelocity(0.0, ANGULAR_VELOCITY,1);
...
bool Turtlebot3Drive::controlLoop()
{
  ...
  switch(turtlebot3_state_num)
  {
    ...
    case TB3_DRIVE_FORWARD:
      updatecommandVelocity(LINEAR_VELOCITY, 0.0, 0);
    ...
    case TB3_RIGHT_TURN:
      if (fabs(prev_tb3_pose_ - tb3_pose_) >= escape_range_)
        ...
        updatecommandVelocity(0.0, -1 * ANGULAR_VELOCITY, 0);
      break;
    ...
    case TB3_LEFT_TURN:
      if (fabs(prev_tb3_pose_ - tb3_pose_) >= escape_range_)
        ...
        updatecommandVelocity(0.0, ANGULAR_VELOCITY, 0);
      break;
